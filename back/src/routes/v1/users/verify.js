...
verify.generateToken = async(req, res, next) => {
  if(req._isAlreadyAnUser){
    ...
    //user is already verified from shopify
    if(req.headers["x-vidiren-shopify-verified"] == 1){
      await new UsersSQL(req._siteId).updatePassword(req.body.email, req.body.password);
      await new UserBasicInfoRedis(req._siteId).updatePassword(req._userId, req.body.password);
