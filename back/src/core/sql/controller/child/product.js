...
const {
  ...
  Menu: {SCHEMA:{FIELDS: MENU_FIELDS, TABLE_NAME: MENU_TABLE_NAME}},
...
const QUERY_BUILDER = {
  GET_DETAIL: (slug) => {
    const query = ` SELECT ${PRODUCT_FIELDS.ID}, ${PRODUCT_FIELDS.NAME}, ${PRODUCT_FIELDS.SLUG}, ${PRODUCT_FIELDS.DESCRIPTION}, ${PRODUCT_FIELDS.SIZE}, ${PRODUCT_FIELDS.PRICE}, ${PRODUCT_FIELDS.SHOPIFY_PRODUCT_ID}
  ...
  GET_PRODUCT_VIDEO: (pid) => {
    const query = ` SELECT v.${VIDEO_FIELDS.SLUG} as videoSlug, mv.${MENU_VIDEO_FIELDS.MENU_ID} as categoryId, m.${MENU_FIELDS.PARENT_ID} as parentCategoryId
    ...
    INNER JOIN ${MENU_VIDEO_TABLE_NAME} as mv on mv.${MENU_VIDEO_FIELDS.VIDEO_ID} = v.${VIDEO_FIELDS.ID}
    INNER JOIN ${MENU_TABLE_NAME} as m on m.${MENU_FIELDS.ID} = mv.${MENU_VIDEO_FIELDS.MENU_ID}
  ...
  GET_VIDEO_DETAIL: (slug) => {
    ...
    INNER JOIN ${MENU_VIDEO_TABLE_NAME} as mv on mv.${MENU_VIDEO_FIELDS.VIDEO_ID} = v.${VIDEO_FIELDS.ID}
    LEFT JOIN ${MENU_TABLE_NAME} as m on m.${MENU_FIELDS.ID} = mv.${MENU_VIDEO_FIELDS.MENU_ID}
  ...
  GET_VIDEO_DETAIL: (slug) => {
    ...
    v.${VIDEO_FIELDS.HLS_MASTER_PUBLIC_URL} as hlsUrl, v.${VIDEO_FIELDS.POSTER} as poster, 
    m.${MENU_FIELDS.ID} as subCategoryId, m.${MENU_FIELDS.PARENT_ID} as catagoryId, md.*
    FROM ${PRODUCT_TABLE_NAME} as P
    LEFT JOIN ${PRODUCT_VIDEO_TABLE_NAME} as PV on P.${PRODUCT_FIELDS.ID} = PV.${PRODUCT_VIDEO_FIELDS.PRODUCT_ID}
    LEFT JOIN ${VIDEO_TABLE_NAME} as v on v.${VIDEO_FIELDS.ID} = PV.${PRODUCT_VIDEO_FIELDS.VIDEO_ID}
