...
if (workbox) {
	(function () {
		...
		// workbox.routing.registerRoute(/(http|https):\/\/(frontend-api-navik.emk.com)(.*?)/, function (args) {
    ...
    // }, 'GET');
		...
		/* Static caching */
		workbox.routing.registerRoute(/^(https|http):\/\/((www|dev|demo)\.emkbeverlyhills\.com|localhost:4003)\/.*\.(woff|woff2|png|jpg|jpeg|svg|gif|ico).*/, new workbox.strategies.CacheFirst({
			cacheName: ASSETS_CACHE,
			plugins: [new workbox.expiration.Plugin({
				maxEntries: 50,
		...
		workbox.routing.registerRoute(/^(https|http):\/\/((www|dev|demo)\.emkbeverlyhills\.com|localhost:4003)\/.*\.(js|css).*/, new workbox.strategies.StaleWhileRevalidate({
			cacheName: ASSETS_CACHE,
			plugins: [new workbox.expiration.Plugin({
				maxEntries: 50,
				maxAgeSeconds: 30*24*60*60 // 30 days
		...
		workbox.routing.setCatchHandler(({event}) => {
			switch (event.request.destination) {
				...
				case 'unknown':
					...
					break;
				case 'image':
					return caches.match(workbox.precaching.getCacheKeyForURL('/images/emk-default.png'));
					break;
				default:
