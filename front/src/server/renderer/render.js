...
export default (request, res, store, storeData, context, routematch, setStatus, isMobile) => {
    ...
    try{
        ...
        let extraprops = { ...routematch.params, status, isMobile, serverRequest:true}
...
function getHeader(extractor, lang, helmet, isMobile, request){
    ...
    return `<!doctype html><html lang="${lang}"><head>${helmet.title.toString()}${helmet.meta.toString()}${helmet.link.toString()}${getAPPIconHTML(lang)}${getPreConnect()}${linkTags}${(process.env.NODE_APP != 'production') ? '<meta name="robots" content="noindex, nofollow" />' :''}
    <link rel="preload" as="font" href="/fonts/HindSiliguri-Medium.ttf" type="font/ttf" crossorigin="anonymous">
